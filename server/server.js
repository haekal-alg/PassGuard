...
app
  .route("/api/user/(|loginInfo|secureNote|creditCard)")
...
db.sequelize
  ...
  .then(() => {
    console.log(
      `[${config.NODE_ENV.toUpperCase()}] Connected and synced to ${
        config.DB_DATABASE
      } DB`
    );
  })
...
app.listen(config.PORT, () => {
  console.log(
    `[${config.NODE_ENV.toUpperCase()}] Server is running on port ${
      config.PORT
    }`
  );
});
