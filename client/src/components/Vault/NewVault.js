...
function NewVault() {
  ...
  async function editLogin(item) {
    if (
      ...
      const encryptedNewLoginName = cipher.aes256Encrypt(
        iv,
        newLoginName.current.value,
        symkey
      ); // encrypt
      newLoginName.current.value = Buffer.from(encryptedNewLoginName).toString(
        "base64"
      ); // encode with base64

      const encryptedNewLoginUsername = cipher.aes256Encrypt(
        iv,
        newUsername.current.value,
        symkey
      ); // encrypt
      newUsername.current.value = Buffer.from(
        encryptedNewLoginUsername
      ).toString("base64"); // encode with base64

      const encryptedNewLoginPassword = cipher.aes256Encrypt(
        iv,
        newPassword.current.value,
        symkey
      ); // encrypt
      newPassword.current.value = Buffer.from(
        encryptedNewLoginPassword
      ).toString("base64"); // encode with base64
      console.log(
        encryptedNewLoginName,
        encryptedNewLoginPassword,
        encryptedNewLoginPassword
      );
      console.log(
      ...
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/api/user/loginInfo?loginInfoId=${item.loginInfoId}&name=${newLoginName.current.value}&username=${newUsername.current.value}&password=${newPassword.current.value}`,
        {
          method: "PATCH",
          headers: {
  ...
  async function editNote(item) {
    if (newNoteName !== "" && newNoteMessage !== "") {
      ...
      newNoteName.current.value =
        Buffer.from(encryptedNewNoteName).toString("base64"); // encode with base64
      ...
      newNoteMessage.current.value = Buffer.from(
        encryptedNewNoteMessage
      ).toString("base64"); // encode with base64
      console.log(item.secureNoteId);

      // send data to server to update data
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/api/user/secureNote?secureNoteId=${item.secureNoteId}&name=${newNoteMessage.current.value}&notes=${newNoteName.current.value}`,
        {
          method: "PATCH",
          headers: {
            "Content-type": "application/json",
